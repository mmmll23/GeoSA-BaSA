...
class InteractionBlock_cls_efficient(nn.Module):
    ...
    def forward(self, x, c, blocks, index, enhancer, bridge, H, W):
...
class GeoSACLIPVisionTransformer(CLIPVisionTransformer):
    def __init__(
        self,
        conv_inplane,
        ...
        **kwargs,
        ...
        self.interactions = nn.Sequential(*[
            InteractionBlock_cls_efficient(dim=self.side_dims, num_heads=16,qkv_bias=True,proj_bias=True,
                                           norm_layer=norm_layer,with_cffn=True, cffn_ratio=0.5)
        ...
        self.conv_1 = nn.Conv2d(self.width, self.side_dims, kernel_size=1, stride=1, padding=0, bias=True)
        self.conv_2 = nn.Conv2d(self.width, self.side_dims, kernel_size=1, stride=1, padding=0, bias=True)
        self.conv_3 = nn.Conv2d(self.width, self.side_dims, kernel_size=1, stride=1, padding=0, bias=True)
        self.conv_4 = nn.Conv2d(self.width, self.side_dims, kernel_size=1, stride=1, padding=0, bias=True)
    ...
    def forward_features(self, x, masks=None):
        ...
        # print(x.shape)  #[1025, 4, 1024]

        outs_multilayer = list()
        for i, layer in enumerate(self.interactions):
            indexes = self.interaction_indexes[i]  # e.g.,[0,5]
            x, c = layer(x, c, self.transformer.resblocks[indexes[0]:indexes[-1] + 1],i,self.enhancer,self.bridge,H, W)
            outs_multilayer.append(x.permute(1, 0, 2)[:, 1:, :].permute(0, 2, 1).view(B, -1, H, W).contiguous())
        # ops = [self.fpn1, self.fpn2, self.fpn3, self.fpn4]
        # for i in range(len(outs_multilayer)):
        #     outs_multilayer[i] = ops[i](outs_multilayer[i])
        ...
        c2 = c2.transpose(1, 2).view(B, -1, H * 2, W * 2).contiguous()  #bs,dim,128,128
        c3 = c3.transpose(1, 2).view(B, -1, H, W).contiguous()   #bs,dim,64,64
        c4 = c4.transpose(1, 2).view(B, -1, H // 2, W // 2).contiguous()  ##bs,dim,32,32
    ...
    def train(self, mode: bool = True):
        ...
        set_requires_grad2(self, ["transformer","class_embedding","positional_embedding","conv1","ln_pre","fpn"])
        set_train2(self, ["transformer","class_embedding","positional_embedding","conv1","ln_pre","fpn"])

    def state_dict(self, destination, prefix, keep_vars):
        state = super().state_dict(destination, prefix, keep_vars)
        keys = [k for k in state.keys() if "transformer" in k or "class_embedding" in k or "positional_embedding" in k or "conv1" in k or "ln_pre" in k or "fpn" in k]
        for key in keys:
            state.pop(key)
