...
class GeoSADinoVisionTransformer(DinoVisionTransformer):
    def __init__(
        self,
        conv_inplane,
        ...
        **kwargs,
        ...
        self.interactions = nn.Sequential(*[
            InteractionBlock_cls_efficient(dim=self.side_dims, num_heads=16,qkv_bias=self.qkv_bias,proj_bias=self.proj_bias,
    ...
    def forward_features(self, x, masks=None):
        ...
        for i, layer in enumerate(self.interactions):
            ...
            x, c = layer(x, c, self.blocks[indexes[0]:indexes[-1] + 1],i,self.enhancer,self.bridge, H, W)
            outs_multilayer.append(x[:, 1:, :].permute(0, 2, 1).view(B, -1, H, W).contiguous())
    ...
    def train(self, mode: bool = True):
        ...
        set_requires_grad2(self, ["blocks","cls_token","pos_embed","mask_token","patch_embed"])
        set_train2(self, ["blocks","cls_token","pos_embed","mask_token","patch_embed"])

    def state_dict(self, destination, prefix, keep_vars):
        state = super().state_dict(destination, prefix, keep_vars)
        keys = [k for k in state.keys() if "blocks" in k or "cls_token" in k or "pos_embed" in k or "mask_token" in k or "patch_embed" in k]
        for key in keys:
            state.pop(key)
